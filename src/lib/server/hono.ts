...
app.get('/api/services', async (c) => {
  try {
    const rows = await railwayEnv.DB.prepare('SELECT * FROM services').all();
...
app.post('/api/leads', async (c) => {
  ...
  const r = await railwayEnv.DB.prepare(
...
app.get('/api/leads', async (c) => {
  ...
  const q = status
    ? railwayEnv.DB.prepare('SELECT * FROM leads WHERE status=? ORDER BY id DESC').bind(status)
    : railwayEnv.DB.prepare('SELECT * FROM leads ORDER BY id DESC');
...
app.get('/api/products', async (c) => {
  ...
  try {
    const rows = await railwayEnv.DB.prepare(sql).bind(...binds).all();
    return c.json(rows.results);
  } catch { return c.json([]); }
});

app.get('/api/products/:id', async (c) => {
  const row = await railwayEnv.DB.prepare('SELECT * FROM products WHERE id=?').bind(c.req.param('id')).first();
  if (!row) return c.json({ error: 'not found' }, 404);
  await railwayEnv.DB.prepare('UPDATE products SET views=views+1 WHERE id=?').bind(c.req.param('id')).run();
  return c.json(row);
});

app.get('/api/groups', async (c) => {
  try {
    const rows = await railwayEnv.DB.prepare('SELECT * FROM preorder_groups ORDER BY id DESC').all();
    return c.json(rows.results);
  } catch { return c.json([]); }
});

app.get('/api/groups/:id/products', async (c) => {
  const rows = await railwayEnv.DB.prepare('SELECT * FROM products WHERE group_id=?').bind(c.req.param('id')).all();
  return c.json(rows.results);
});
...
app.put('/api/groups/:id', async (c) => {
  ...
  const b = await c.req.json();
  const old: any = await railwayEnv.DB.prepare('SELECT * FROM preorder_groups WHERE id=?').bind(id).first();
  await railwayEnv.DB.prepare(
  ...
  if (b.status && old && old.status !== b.status) {
    const orders = await railwayEnv.DB.prepare('SELECT DISTINCT customer_phone FROM preorder_orders WHERE group_id=?').bind(id).all<{ customer_phone: string }>();
    for (const o of orders.results) {
      await sendWhatsApp(c.env, o.customer_phone, `STARC.CM - Groupe "${old.title}" est maintenant: ${b.status}`);
    }
    if (b.status === 'ARRIVED') {
      const rows = await railwayEnv.DB.prepare('SELECT customer_phone, remaining_xaf FROM preorder_orders WHERE group_id=?').bind(id).all<{ customer_phone: string; remaining_xaf: number }>();
...
app.post('/api/orders', async (c) => {
  ...
  for (const it of b.items) {
    const p: any = await railwayEnv.DB.prepare('SELECT * FROM products WHERE id=?').bind(it.product_id).first();
    if (!p) continue;
  ...
  if (b.delivery_mode === 'domicile') {
    const zone: any = await railwayEnv.DB.prepare('SELECT fee FROM delivery_zones WHERE city=?').bind(b.customer.city).first();
  ...
  const orderRes = await railwayEnv.DB.prepare(
  ...
  for (const it of items) {
    ...
    if (it.is_preorder) {
      await railwayEnv.DB.prepare('UPDATE products SET moq_current = moq_current + ? WHERE id=?').bind(it.qty, it.id).run();
    }
  }
  ...
  await railwayEnv.DB.prepare('UPDATE preorder_orders SET receipt_number=? WHERE id=?').bind(receiptNumber, orderId).run();
  await railwayEnv.DB.prepare('UPDATE customers SET total_orders = total_orders + 1, total_spent = total_spent + ? WHERE id=?').bind(total, customerId).run();
...
app.get('/api/orders', async (c) => {
  const rows = await railwayEnv.DB.prepare('SELECT * FROM preorder_orders ORDER BY id DESC LIMIT 500').all();
  return c.json(rows.results);
});

app.get('/api/orders/:id', async (c) => {
  return c.json(await railwayEnv.DB.prepare('SELECT * FROM preorder_orders WHERE id=?').bind(c.req.param('id')).first());
});

app.put('/api/orders/:id/status', async (c) => {
  const b = await c.req.json();
  await railwayEnv.DB.prepare('UPDATE preorder_orders SET status=? WHERE id=?').bind(b.status, c.req.param('id')).run();
  const o: any = await railwayEnv.DB.prepare('SELECT * FROM preorder_orders WHERE id=?').bind(c.req.param('id')).first();
  if (o) await sendWhatsApp(c.env, o.customer_phone, `Statut commande ${o.receipt_number}: ${b.status}`);
  return c.json({ ok: true });
});

app.get('/api/receipt/:id', async (c) => {
  return c.json(await railwayEnv.DB.prepare('SELECT * FROM receipts WHERE order_id=? OR id=?').bind(c.req.param('id'), c.req.param('id')).first());
});

app.get('/api/receipts', async (c) => {
  const q = c.req.query('q');
  let stmt = railwayEnv.DB.prepare('SELECT * FROM receipts ORDER BY id DESC LIMIT 500');
  if (q) stmt = railwayEnv.DB.prepare('SELECT * FROM receipts WHERE receipt_number LIKE ? ORDER BY id DESC').bind(`%${q}%`);
  return c.json((await stmt.all()).results);
});

app.get('/api/customers', async (c) => {
  const q = c.req.query('q');
  let stmt = railwayEnv.DB.prepare('SELECT * FROM customers ORDER BY id DESC LIMIT 500');
  if (q) stmt = railwayEnv.DB.prepare('SELECT * FROM customers WHERE phone LIKE ? OR name LIKE ? ORDER BY id DESC').bind(`%${q}%`, `%${q}%`);
  return c.json((await stmt.all()).results);
});

app.get('/api/customers/:phone', async (c) => {
  return c.json(await railwayEnv.DB.prepare('SELECT * FROM customers WHERE phone=?').bind(c.req.param('phone')).first());
});

app.get('/api/inventory', async (c) => {
  const rows = await railwayEnv.DB.prepare(
...
app.post('/api/inventory/movement', async (c) => {
  ...
  await railwayEnv.DB.prepare('UPDATE inventory SET qty_instock = qty_instock + ? WHERE product_id=?').bind(delta, b.product_id).run();
  const inv: any = await railwayEnv.DB.prepare('SELECT * FROM inventory WHERE product_id=?').bind(b.product_id).first();
...
app.get('/api/inventory/movements/:product_id', async (c) => {
  const rows = await railwayEnv.DB.prepare('SELECT * FROM inventory_movements WHERE product_id=? ORDER BY id DESC LIMIT 200').bind(c.req.param('product_id')).all();
  return c.json(rows.results);
});

app.get('/api/dashboard/stats', async (c) => {
  try {
    const db = railwayEnv.DB;
...
app.post('/api/upload', async (c) => {
  ...
  await railwayEnv.FILES.put(key, file.stream(), { httpMetadata: { contentType: file.type } });
...
app.get('/api/file/*', async (c) => {
  ...
  const obj = await railwayEnv.FILES.get(key);
...
app.get('/api/my-orders', async (c) => {
  ...
  const orders = await railwayEnv.DB.prepare(
  ...
  const receipts = await railwayEnv.DB.prepare('SELECT * FROM receipts WHERE customer_id=(SELECT id FROM customers WHERE phone=?) ORDER BY id DESC').bind(phone).all();
  const files = await railwayEnv.DB.prepare('SELECT * FROM project_files WHERE customer_id=(SELECT id FROM customers WHERE phone=?) ORDER BY id DESC').bind(phone).all();
  const sourcing = await railwayEnv.DB.prepare('SELECT * FROM sourcing_requests WHERE customer_phone=? ORDER BY id DESC').bind(phone).all();
  const customer = await railwayEnv.DB.prepare('SELECT * FROM customers WHERE phone=?').bind(phone).first();
  const wallet = await railwayEnv.DB.prepare('SELECT * FROM fidelity_wallet WHERE phone=?').bind(phone).first();
...
app.get('/api/sourcing-requests', async (c) => {
  return c.json((await railwayEnv.DB.prepare('SELECT * FROM sourcing_requests ORDER BY id DESC').all()).results);
});
...
app.put('/api/sourcing-requests/:id', async (c) => {
  ...
  if (b.status === 'quoted') {
    const s: any = await railwayEnv.DB.prepare('SELECT * FROM sourcing_requests WHERE id=?').bind(c.req.param('id')).first();
...
app.post('/api/whatsapp/webhook', async (c) => {
  ...
  if (phone && text) {
    const cust: any = await railwayEnv.DB.prepare('SELECT id FROM customers WHERE phone=?').bind(phone).first();
    if (cust) {
      await railwayEnv.DB.prepare("INSERT INTO interactions (customer_id, type, notes, created_by) VALUES (?, 'whatsapp', ?, 'client')").bind(cust.id, text).run();
    }
...
app.get('/api/whatsapp/logs', async (c) => {
  return c.json((await railwayEnv.DB.prepare('SELECT * FROM whatsapp_logs ORDER BY id DESC LIMIT 200').all()).results);
});

app.get('/api/delivery-zones', async (c) => {
  return c.json((await railwayEnv.DB.prepare('SELECT * FROM delivery_zones').all()).results);
});

app.post('/api/delivery-zones', async (c) => {
  const b = await c.req.json();
  await railwayEnv.DB.prepare('INSERT INTO delivery_zones (city, fee) VALUES (?, ?) ON CONFLICT(city) DO UPDATE SET fee=excluded.fee').bind(b.city, b.fee).run();
  return c.json({ ok: true });
});

app.get('/api/team', async (c) => {
  return c.json((await railwayEnv.DB.prepare('SELECT * FROM team_members').all()).results);
});
...
app.post('/api/settings', async (c) => {
  const b = await c.req.json();
  if (b.shipping_rate) await railwayEnv.KV.put('shipping_rate', String(b.shipping_rate));
  if (b.customs_rate) await railwayEnv.KV.put('customs_rate', String(b.customs_rate));
  return c.json({ ok: true });
});
...
app.get('/api/admin/products', async (c) => {
  const rows = await railwayEnv.DB.prepare('SELECT * FROM products ORDER BY id DESC').all();
  return c.json(rows.results);
});
...
app.delete('/api/admin/products/:id', async (c) => {
  await railwayEnv.DB.prepare('DELETE FROM products WHERE id=?').bind(c.req.param('id')).run();
  return c.json({ ok: true });
});
...
app.delete('/api/admin/services/:id', async (c) => {
  await railwayEnv.DB.prepare('DELETE FROM services WHERE id=?').bind(c.req.param('id')).run();
  return c.json({ ok: true });
});
...
app.get('/api/blog', async (c) => {
  try {
    const rows = await railwayEnv.DB.prepare('SELECT id, slug, title, created_at FROM blog ORDER BY id DESC').all();
    return c.json(rows.results);
  } catch { return c.json([]); }
});

app.get('/api/blog/:slug', async (c) => {
  const row = await railwayEnv.DB.prepare('SELECT * FROM blog WHERE slug=?').bind(c.req.param('slug')).first();
  return c.json(row ?? {});
});
...
app.delete('/api/admin/blog/:id', async (c) => {
  await railwayEnv.DB.prepare('DELETE FROM blog WHERE id=?').bind(c.req.param('id')).run();
  return c.json({ ok: true });
});
...
app.put('/api/admin/pages/home', async (c) => {
  const b = await c.req.json();
  await railwayEnv.KV.put('home_hero', JSON.stringify(b));
  return c.json({ ok: true });
});
