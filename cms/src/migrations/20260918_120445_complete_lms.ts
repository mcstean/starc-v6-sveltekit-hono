...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE INDEX \`admins_avatar_idx\` ON \`admins\` (\`avatar_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`courses\` (
  	...
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	FOREIGN KEY (\`instructor_id\`) REFERENCES \`admins\`(\`id\`) ON UPDATE no action ON DELETE set null,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`lessons_slug_idx\` ON \`lessons\` (\`slug\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`enrollments_student_idx\` ON \`enrollments\` (\`student_id\`);`)
  await db.run(sql`CREATE INDEX \`enrollments_course_idx\` ON \`enrollments\` (\`course_id\`);`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_users\`("id", "name", "role", "company", "updated_at", "created_at", "email", "reset_password_token", "reset_password_expiration", "salt", "hash", "login_attempts", "lock_until") SELECT "id", "name", "role", "company", "updated_at", "created_at", "email", "reset_password_token", "reset_password_expiration", "salt", "hash", "login_attempts", "lock_until" FROM \`users\`;`)
  ...
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`admins_id\` integer REFERENCES admins(id);`)
  ...
  await db.run(sql`ALTER TABLE \`payload_preferences_rels\` ADD \`admins_id\` integer REFERENCES admins(id);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_media\`("id", "alt", "caption", "updated_at", "created_at", "url", "thumbnail_u_r_l", "filename", "mime_type", "filesize", "width", "height", "focal_x", "focal_y") SELECT "id", "alt", "caption", "updated_at", "created_at", "url", "thumbnail_u_r_l", "filename", "mime_type", "filesize", "width", "height", "focal_x", "focal_y" FROM \`media\`;`)
  await db.run(sql`DROP TABLE \`media\`;`)
  await db.run(sql`ALTER TABLE \`__new_media\` RENAME TO \`media\`;`)
  await db.run(sql`CREATE INDEX \`media_updated_at_idx\` ON \`media\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`media_created_at_idx\` ON \`media\` (\`created_at\`);`)
